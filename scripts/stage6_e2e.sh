...
e2e_directory="$(mktemp -d -t chess-workbench-stage6-XXXXXX)"
api_port="${CHESS_WORKBENCH_STAGE6_E2E_API_PORT:-18800}"
web_port="${CHESS_WORKBENCH_STAGE6_E2E_WEB_PORT:-15174}"
api_pid=""
web_pid=""
...
cd "$project_root"
export CHESS_WORKBENCH_DATABASE_URL="sqlite+aiosqlite:///$e2e_directory/stage6.db"
export CHESS_WORKBENCH_SOURCE_STORAGE_ROOT="$e2e_directory/data"
export CHESS_WORKBENCH_STOCKFISH_PATH="$project_root/backend/tests/fixtures/fake_uci_engine.py"
export CHESS_WORKBENCH_SYZYGY_PATH="$project_root/backend/tests/fixtures/syzygy"

uv run --project backend --locked alembic -c backend/alembic.ini upgrade head \
  >"$e2e_directory/migration.log" 2>&1

setsid env \
  CHESS_WORKBENCH_SERVICE_NAME="chess-workbench-stage6-e2e" \
...
if ! uv run --project backend --locked python scripts/assert_health.py \
  --service "chess-workbench-stage6-e2e" \
...
CHESS_WORKBENCH_E2E_BASE_URL="http://127.0.0.1:$web_port" \
  "${pnpm_command[@]}" --dir frontend exec playwright test e2e/engine-workspace.spec.ts
test_rc=$?
set -e
if [[ "$test_rc" -ne 0 ]]; then
  echo "Stage 6 API log:"
  sed -n '1,240p' "$e2e_directory/api.log"
  echo "Stage 6 frontend log:"
  sed -n '1,240p' "$e2e_directory/web.log"
fi
exit "$test_rc"
