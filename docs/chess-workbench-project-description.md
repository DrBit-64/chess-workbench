...
- 对扫描件执行 OCR；
- 识别正文和棋谱区块；
- 将章节转换为结构化候选内容（参见 ADR 0006：Block 序列）；
- 审核低置信度结果；
- 将合法分支导入知识图；
- 上传或指定视频文件；
- 提取音频和关键帧；
...
- 每个章节的内容是一个有序的 Block 序列（参见 ADR 0006）：小节标题、叙事段落、着法序列和知识笔记交替出现，保持原书叙事结构。
- 着法序列展开为 CourseOccurrence 链，中间棋盘随走子同步更新。
- KnowledgeNote 包含完整的 markdown 内容，通过 SourceSpan 引用原书页码或视频时间。
- 章节嵌套用 Module.parent_id 树实现。
- 原书中的棋盘图不单独保留——OCR 提取着法和 FEN 后丢弃原图，交互界面用中心可交互棋盘替代。
